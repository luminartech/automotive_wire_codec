...
- **Zero runtime dependencies** — \[`Sink`\] is a tiny crate-owned trait, not a
  dependency on an external I/O crate. \[`SliceSink`\], \[`CountingSink`\], and
  \[`Limited`\] cover buffer encoding, sizing, and transport-maximum enforcement
  out of the box.
...
trait defaults and leaf helpers construct errors generically. Encode-side failures
surface as \[`WriteError`\]; the \[`Encode`\] error bound is `From<WriteError>` — one
`From` impl covers the sink, the fixed-width helpers, and the variable-width
\[`write_be_uint`\]. Because `From<T> for T` holds reflexively, a type whose only
failure mode is the write path can set `type Error = WriteError` directly, with no
bespoke error type at all. Because the L1 error implements these `From` bounds,
...
\[`SliceSink`\] wraps `&mut [u8]` as a \[`Sink`\], an outer protocol serializes an inner
value directly into one buffer — no second allocation or copy:

```rust
use automotive_wire_codec::{write_u32_be, Encode, Sink, SliceSink, WriteError};
...
impl Encode for Header {
    ...
    fn encode(&self, sink: &mut impl Sink) -> Result<usize, Self::Error> {
        write_u32_be(sink, self.payload_len)
    }
}
struct Inner(u32);
...
let mut sink = SliceSink::new(&mut tx_buf);    // one buffer
let mut total = header.encode(&mut sink)?;     // writes header, advances the sink
total += inner.encode(&mut sink)?;             // writes inner into the remainder
assert_eq!(total, 8);
Ok::<(), WriteError>(())
```
...
use automotive_wire_codec::{
    read_u16_be, write_u16_be, Decode, Encode, Incomplete, Limited, Sink, SliceSink,
    TrailingBytes, WriteError,
};
...
impl From<WriteError> for PingError {
...
impl Encode for Ping {
    type Error = PingError;
    fn encode(&self, sink: &mut impl Sink) -> Result<usize, Self::Error> {
        Ok(write_u16_be(sink, self.session_id)?)
    }
}

fn main() -> Result<(), PingError> {
    // Round-trip: encode into a buffer bounded to the transport's maximum,
    // then decode it back.
    let ping = Ping { session_id: 0x1234 };
    let mut buf = [0u8; 2];
    let max_len = buf.len();
    let mut sink = Limited::new(SliceSink::new(&mut buf), max_len);
    ping.encode(&mut sink)?;
